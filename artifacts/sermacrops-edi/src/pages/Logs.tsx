    ...
    <div className="p-4 sm:p-6 max-w-5xl mx-auto">
      <div className="flex items-center justify-between mb-6 gap-3 flex-wrap">
        ...
        <Select value={entityTypeFilter} onValueChange={setEntityTypeFilter}>
          <SelectTrigger data-testid="select-log-entity-type" className="w-44">
      ...
      <div className="bg-card border border-card-border rounded-lg overflow-hidden">
        {/* Desktop header row */}
        <div className="hidden sm:grid px-5 py-3 bg-muted/40 border-b border-border grid-cols-12 gap-4">
        ...
        <div className="divide-y divide-border">
          {logs?.map(log => {
            ...
            const detailsText = details ? Object.entries(details).map(([k, v]) => `${k}: ${v}`).join(", ") : null;
            return (
              <div key={log.id} data-testid={`log-row-${log.id}`} className="hover:bg-muted/30 transition-colors">
                {/* Mobile card layout */}
                <div className="sm:hidden px-4 py-3 space-y-2">
                  ...
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    <span className="text-xs font-mono text-muted-foreground truncate max-w-[160px]">{log.entityId}</span>
                  </div>
                  {detailsText && (
                    <p className="text-xs text-muted-foreground truncate">{detailsText}</p>
                  )}
                </div>

                {/* Desktop grid layout */}
                <div className="hidden sm:grid px-5 py-3 grid-cols-12 gap-4 items-center">
                  ...
                  <div className="col-span-3">
                    {detailsText ? (
                      <span className="text-xs text-muted-foreground truncate block">{detailsText}</span>
                    ) : (
